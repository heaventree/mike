/*
  ...
  This migration sets up the complete database schema for the Mike AI Legal Platform.

  ## Summary
  Creates all tables needed for users, projects, documents, workflows, chats, and tabular reviews.

  ## Tables Created
  - `user_profiles` - User metadata (display name, org, tier, credits, model preferences)
  - `user_api_keys` - Encrypted per-user LLM API keys (claude, gemini, openai)
  - `projects` - Project containers for documents
  - `project_subfolders` - Nested folder structure within projects
  - `documents` - Individual document records
  - `document_versions` - Version history for documents
  - `document_edits` - Tracked changes/edits in documents
  - `workflows` - Reusable extraction/analysis templates
  - `hidden_workflows` - User's hidden workflow list
  - `workflow_shares` - Workflow sharing/permissions
  - `chats` - Conversation threads
  - `chat_messages` - Individual messages in chats
  - `tabular_reviews` - Data extraction tables
  - `tabular_cells` - Individual cell data in tabular reviews
  - `tabular_review_chats` - Discussion threads within tabular reviews
  - `tabular_review_chat_messages` - Messages in review chats

  ## Security
  - All application data tables have anon/authenticated roles revoked
  - Access goes through backend API with service role key
  - A trigger auto-creates user_profiles on signup
*/
